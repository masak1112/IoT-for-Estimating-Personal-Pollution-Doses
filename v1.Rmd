...
df_outdoor_indoor = function(DATE,POLLUTANT_CODE){
        ...
        m=dbDriver("MySQL")
        con<-dbConnect(m,user="cmadrid",password="tian.2016",dbname="madrid_pollution",host="138.100.82.183")
        start = DATE
        end = DATE
        ...
        if(file.exists(paste0(df_path,"pollutant_code",POLLUTANT_CODE,".RData")))
                ...
                t_lat <- gsub("\xba","°",p$lat)
                p$lat <- as.numeric(char2dms(t_lat, chd = "°", chm = "'", chs='"'))
                t_lon <- gsub("\xba","°",p$lon)
                p$lon <- as.numeric(char2dms(t_lon, chd = "°", chm = "'", chs='"'))
...
remove.packages(c("ggmap", "ggplot2"))
